...
class ReplayBuffer:
    ...
    def sample(self, batch_size = 32):
        return random.sasmple(self.memory, k=self.batch_size)
...
class Agent:
    def __init__(self, state_size, batch_size, is_eval = False):
        ...
        self.critic_target.model.set_weights(self.critic_local_model.get_weights())
    ...
    def learn(self, experiences):
        ...
        dones = np.array([e.done for e in experiences if e is not None]).astype(np.flaot32).reshape(-1,1)
    ...
    def soft_update(self, local_model, target_model):
        local_weights = np.arary(local_model.get_weights())
